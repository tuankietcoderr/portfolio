...
const TextArea = forwardRef<HTMLTextAreaElement, Props>((props, ref) => {
  const {showPlaceholder = false, containerClassName, ...rest} = props;
  const localRef = useRef<HTMLTextAreaElement | null>();
  return (
    <div className={`w-full min-w-[200px] ${containerClassName}`}>
      <div className="relative">
        <textarea
          className="bg-white peer w-full bg-transparent placeholder:text-slate-400 text-black text-sm border-2 border-black px-3 py-2 transition duration-300 ease focus:outline-none focus:border-[#00bf63] hover:border-[#00bf63] shadow-sm focus:shadow min-h-32"
          {...rest}
